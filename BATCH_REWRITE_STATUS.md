...
- 机器扫描：1331 个章文件；336 章 ERROR，77 章 WARN。全书机器门仍未过，这只是基线观测，不等于本轮绿段的发布证明。
...
- 本轮追加 ch894、ch895 后的最新刷新值为：304 个唯一 lint 错误号、5 个当前候选文件未过完整发布门，机器扫描为 342 章 ERROR、77 章 WARN；上一条 306/7 为 ch892、ch893 提交前记录。
- 本轮追加 ch899、ch901 后的最新刷新值为：302 个唯一 lint 错误号、3 个当前候选文件未过完整发布门，机器扫描为 340 章 ERROR、77 章 WARN；上一条 304/5 为 ch894、ch895 提交前记录。
- 本轮追加 ch907、ch926 后的最新刷新值为：300 个唯一 lint 错误号、1 个当前候选文件未过完整发布门，机器扫描为 338 章 ERROR、77 章 WARN；上一条 302/3 为 ch899、ch901 提交前记录。
- 本轮追加 ch932 后的最新刷新值为：299 个唯一 lint 错误号、0 个当前候选文件未过完整发布门，机器扫描为 337 章 ERROR、77 章 WARN；上一条 300/1 为 ch907、ch926 提交前记录。全量扫描随后清理了历史 alternate `ch881-灶边雪.md`，当前机器基线降为 336 章 ERROR、77 章 WARN；canonical `ch881-林崇看.md` 仍为已通过主编门。
- alternate 分支 `ch881-灶边雪.md` 已明确标注 `branch: alternate` 并由主编重写到 1600 汉字，独立 strict editorial、安全与硬线门通过；它不替代 canonical `ch881-林崇看.md`。
...
- `ch803-苏挽端糖.md`：Claude 实际改写但留下 3 处 wall_formula；主编重写苏挽核对旧糖纸、药糖与叶蜡，拒绝替阿湄传话，带物证去药庐，正文 1595 汉字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch804-林彻看林夙.md`：Claude 超时未改目标且旧模板循环；主编重写林彻在林崇逼问下把林夙写进家册，并用叶家半枚印留下不可逆证词，正文 1713 汉字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch805-林叙等.md`：Claude 实际改写但留下方向、wall、物象位置与自指回环；主编重写林叙删掉索取回报的旧信，只把原页事实放到东书房门外，正文 1559 汉字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch807-两个名字.md`：Claude 流式中断留下半成稿；主编重写苏挽带回林家两名账页，阿湄拆出叶家挑拨蓝线，林夙要求林彻亲自面对，正文 1643 汉字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch808-林彻站.md`：Claude 实际改写但留下 23 处 wall_formula；主编重写林彻读到“请亲自来”，不让林崇或周平代答，在纸背写“我到”并出门，正文 1537 汉字，最终 lint、公式扫描、strict editorial 与 safety PASS。
...
- `ch814-林彻看林夙.md`：Claude 实际改写但 strict metadata 未过且独立字数仅 1267 字；主编补足林彻与林崇围绕裂弓归属的追问，保留林夙交弓、红绳不遮裂口的不可逆选择，正文 1506 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch815-阿湄那一刀.md`：Claude worker 超时未改目标，旧稿命中方向、自指与物象位置公式；主编重写为阿湄在旧档室切断叶家蓝线，留下两端线头与叶形蜡片，正文 1766 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch816-林崇信.md`：Claude 首稿保留大量方向、物象槽位与自指公式；主编重写为林崇、林夙、余伯围绕裂弓交接纸落名，父亲留下“林崇见”而不以家印替人求情，正文 1521 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch817-三人迎上.md`：Claude 首稿仍有墙式和物象回环；主编保留闭门、灵牌、蓝线与叶形蜡片，重写三人先记后动并由门内落出旧铜钥匙，确认叶家仍在等阿湄，正文 1696 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch818-林彻站.md`：Claude 到时限且未形成可收稿 metadata；主编重写为王姨娘要求林彻替宗祠写“林夙未归”，林彻改留裂弓亲见并拒绝交信，保留旧玉片与兄弟门口对峙，正文 1687 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch819-凌朔不动.md`：Claude 首稿独立公式门抓到 22 处 self_claim，且仍把凌朔写成重复灯/方向模板；主编重写为沈疏桐带来林夙急报，凌朔撕开赤渊封口并发出先救回令，正文 1552 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch820-裴无咎看.md`：Claude 超时未改目标；主编保留包子、粥、灯和叶牌的消息链，重写裴无咎拒绝守令、扯下偏东封条并亲眼确认林夙仍有呼吸，正文 1524 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch822-沈疏桐搁.md`：Claude 首稿约 1450 字且 strict metadata 未过；主编重写沈疏桐以纸人、旧玉扣和未赴斋室回信设局，由凌朔带回赤渊，明确她把命从斋室门口撤出而非接受劝阻，正文 1683 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch823-苏挽端糖.md`：Claude 首稿命中方向/墙式公式；主编重写苏挽接糖纸证物、补记“未验”并接过余伯旧笔，补足旧纸交接现场，正文 1622 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch824-不在设计里.md`：Claude 虽获 runner PASS，但正文/review 仍有自指和位置模板；主编重写叶观澜面对林夙倒下、裴无咎护字与苏挽破封，改为等、备轿、亲自去偏东，正文 1503 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch825-林叙等.md`：Claude 超时留下半稿且 strict metadata 未过；主编重写林叙以粥碗、旧钥匙和小簿完成不记人情的交接，林窈带钥匙去偏东，正文 1550 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch826-还有气.md`：Claude 虽完成改写但命中 117 处物象槽位、26 处自指与方向公式；主编整章重写红墙下确认林夙呼吸、侧门取门板与护字副本，避开未成年角色暧昧身体描写，正文 1553 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch827-阿湄背.md`：Claude 首稿命中 6 处墙式公式与 22 处自指；主编重写门板卡住后的搬运阻力，让阿湄收刀背林夙、苏挽压伤口、林窈带路入偏房，正文 1513 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch828-林窈跟.md`：Claude runner lint 过但 strict 未过；主编重写林窈用旧钥匙开药箱、关侧门并当面喊哥哥醒来，把跟随变成承担，正文 1637 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch829-苏挽在.md`：Claude 首稿 lint 过但 strict metadata 未过；主编重写苏挽在偏房记录“看见的事实”，把旧笔和冬账留给林窈，保留林夙呼吸的未决状态，正文 1532 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch830-林叙看.md`：Claude 超时且未形成可收稿目标；主编重写林叙以红土实报、林崇旧秤和松开的钥匙绳确认林夙尚活，不把救命记成人情，正文 1532 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch831-林崇看.md`：Claude 接近下限且 strict metadata 未过；主编重写林崇以偏东实报、铜秤和姜粥逼林彻面对林夙尚活，收掉旧笑，正文 1507 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch833-苏挽端糖.md`：Claude 未形成有效改写且保留 37 处方向公式；主编重写苏挽以三块药糖、封牌和旧纸完成不以物换账的交接，正文 1558 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch834-不接.md`：Claude 改写留下 6 处 wall_formula 且场景循环；主编重写叶观澜读到林夙仍能服药的药庐急报，拒绝接人，改用换路送药保护药庐，正文 1561 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch835-林叙等.md`：Claude 改写留下自指与 wall_formula 回环；主编重写林叙收到药庐留门短报，在牛阿大与林窈面前放下照料欠账，保留等待，正文 1647 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch836-林崇信.md`：Claude 超时且未改目标；主编重写林崇收到林夙仍能服药的药庐短报，拒绝以家印召回，拆分原报与亲见记录并留下不发召，正文 1526 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch837-灶边雪.md`：Claude 场景可读但 strict editorial 因缺 review/证据拒收；主编复核并重构灶火、送柴、送粥与柴房门槛事件链，补齐来处保护与真实 review，正文 1502 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch838-林彻站.md`：Claude 保留 44 处方向公式与 41 处物象回环；主编重写林彻面对封门牌与冬账，拒绝封山搜林夙，只留下西门未见记录，正文 1709 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch839-苏挽在.md`：Claude 保留 36 处物象槽位回环且称呼与关系错误；主编重写阿湄递出药庐实报、苏挽分开已知与未验、林窈送糖不入账，最终留在药庐门外，正文 1551 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch840-林叙看.md`：Claude 保留 1 处方向公式与 39 处物象回环；主编重写林叙面对林崇留给林夙的短笺，设置送达、经手、收件空栏，最终不替林夙补名，正文 1550 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch843-苏挽端糖.md`：Claude 基础 lint 通过但缺独立 review 证据；主编重写苏挽端三块药糖，在余伯院分开账与人情，再与阿湄用糖纸、蓝泥、鞋齿和空药碗验证门外来客，正文 1549 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch844-林彻看林夙.md`：Claude 留下 23 处方向公式、5 处 wall 公式、44 处物象槽位和 18 处自指回环；主编重写林彻与林崇核对收档边界，林夙带药庐实报站在门外，正文 1629 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch845-林叙等.md`：Claude 基础 lint 通过但缺严格 review 证据；主编重写林叙收到林夙未入府实报，加入林窈核对见闻与牛阿大准备送信的冲突，最后撤掉代答的信，只留下“等林夙回话”，正文 1549 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch846-林崇信.md`：Claude 在 420 秒窗口内超时且未改目标，原稿残留 37 处方向、16 处自我修复、76 处物象位置和 35 处自指回环；主编重写林崇核对门报与药庐报、把家印与短信分开，补上家谱空栏与不封口的送信决定，正文 1553 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch847-灶边雪.md`：Claude 在 420 秒窗口内超时且未改目标，原稿与同批模板重复回环；主编重写无封信随药包到后厨，老仆、牛阿大与林窈分开药包、信纸、经手和收件，正文 1510 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch848-林彻站.md`：Claude 首稿有可用的“信未收”场景但缺 review/score，且与前批一样过度复述物件；主编重写周平带回未收短信，林彻分开药、信、经手与收件，王姨娘明确父亲的字不等于林彻的责任，正文 1500 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch849-苏挽在.md`：Claude 首稿残留 5 处 wall/自指公式且误写九月天气；主编重写苏挽在余伯院核对药庐门报、阿湄旧帕、林窈糖纸和旧笔，只把可见事实入账，正文 1544 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch850-林叙看.md`：Claude 首稿命中 2 处 wall 公式、30 处物象槽位并未通过基础门；主编重写林叙核对药包离开石阶、短笺未收与家印边界，保留“收件：空白”，正文 1619 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch852-灶边.md`：Claude 首稿命中方向、自我修复与 46 处物象槽位回环，且正文不足 1500 字；主编重写林窈送无名饭、牛阿大核验门边、老仆把“出灶，未验”刻入木牌，正文 1634 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch853-苏挽端糖.md`：Claude 首稿命中 4 处来处/方向公式、2 处 wall 公式且正文仅 1383 字；主编重写苏挽核对灶牌、糖水印与卯时门边记录，正文 1502 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch854-林彻看林夙.md`：Claude 未改目标，原稿保留 37 处方向、16 处自我修复、76 处物象槽位和 35 处自称回环；主编重写林夙在东院门槛亲落“我看见了纸，没收纸”，正文 1521 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch855-林叙等.md`：Claude 未改目标，原稿保留 37 处方向、16 处自我修复、76 处物象槽位和 35 处自称回环；主编重写林叙接住“见纸不收”抄件，核对灶房门边实报与林窈空名，正文 1520 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch856-林崇信.md`：Claude 首稿保留 3 处来处公式、19 处自称回环且 review 证据失败；主编重写林夙亲取药包、短笺留秤盘，林崇分卷并把“必要”写入家书，正文 1545 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch857-不收回.md`：Claude 首稿虽完成改写，但命中未成年角色安全边界且 review/score 证据不足；主编重写药庐家书、空收件登记与林夙取药不取信，正文 1538 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch511-林崇看.md`：Claude 首稿丢失完整 frontmatter 且保留方向公式；主编恢复账房连续性，将林彻第三页留白、茶渍证据、回条与旧笔交接落到现场，正文 1524 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch552-灶边.md`：Claude worker 超时且未改动原 89 字占位；主编重写苏挽抄“叶清梧·还”、余伯交旧笔、阿湄转交帕子与糖纸并接明日南城，正文 1554 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch561-林崇看.md`：Claude 机器门 PASS，但首稿仍有高频模板公式；主编独立重写余伯空栏、林叙新绳见证、林崇旁纸落名边界与新笔留灯，正文 1540 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 原回执保留 PASS。
- `ch562-灶边.md`：Claude 首稿命中 4 处 wall 公式且正文不足 1500 字；主编重写阿湄拒接叶观澜的笔、药庐出入簿与末行空方，正文 1504 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch571-林崇看.md`：Claude 首稿丢失 season、hook、ships、review、score 等关键元数据；主编重写林崇分送旧账与灰纸、牛阿大分袋携物并留下量尺，正文 1507 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch581-林崇看.md`：Claude 首稿超时且保留填充句、缺 review；主编重写林崇核对林夙“北门只记到场”、保留第三页留白、记录林窈带糖玉出门，正文 1500 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch584-叶先生的车.md`：Claude 首稿保留 17 处方向、3 处 wall 与 23 处自称回环；主编重写北门门账、车轮未入、阿湄传话和空帕留门槛，正文 1520 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch591-林崇看.md`：Claude 首稿命中 38 处 motif_slot 并被 strict editorial 拒收；主编重写林崇核对冬账空栏、拆分北门门账与口述来源，让林夙只为亲见落名，正文 1630 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch592-灶边.md`：Claude 首稿 lint 失败，保留 13 处方向与 38 处 motif_slot；主编重写苏挽接酉时火班、分开墨帕来源与糖碟送达，正文 1528 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch596-林崇信.md`：Claude 首稿虽过基础 lint，但 strict editorial 拒收其元数据与自证语气；主编重写退婚书代呈、林彻未落名、红印与送达条，正文 1564 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
...
- `ch751-林崇看.md`：Claude 首稿命中 10 处 wall 公式；主编重写三只纸格、半个叶字、来源分栏与不代补姓名的选择，正文 1557 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch806-林崇信.md`：Claude worker 超时且留下元数据与证据门不完整的半稿；主编重写林叙原信、家册青印账页与苏挽转递林夙的分开交接，正文 1554 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch821-林崇看.md`：Claude 首稿缺完整元数据且使用灯自己亮等禁用表达；主编重写灶房铜灯、家祭处封蜡与未拆信交接，正文 1826 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch832-灶边.md`：Claude 首稿缺完整元数据且含禁用模板；主编重写余伯灶膛、苏挽核对旧笔、封牌分列与三份交接记录，正文 1775 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch841-林崇看.md`：Claude 首稿缺完整元数据且带退婚书模板回环；主编重写林叙退回原件、林彻见件不签与青田石印只作收存，正文 1714 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch842-灶边.md`：Claude 首稿缺完整元数据且带大量灶火模板；主编重写阿湄铜秤、余伯留火与苏挽药糖称量，把药糖和收件分开，正文 1631 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch851-林崇看.md`：Claude 首稿缺完整元数据并命中 8 处 wall 公式；主编重写林叙第七页、山门雪脚印、茶棚口述与未验木牌，正文 1526 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch858-林彻站.md`：Claude runner 原始 PASS 但保留 9 处自己回环且压缩原稿过多；主编保留父子站立、周平先饮茶、苏挽留糖与站字原纸，独立重写为正文 1508 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
- `ch863-苏挽端糖.md`：Claude 首稿命中 16 处自指、2 处 wall 且 evidence 元数据不合格；主编重写苏挽药碗与糖碟分栏、林窈糖纸圆印、枇杷核和未收栏，正文 1530 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch864-林彻看林夙.md`：Claude 首稿基础 lint 通过但 strict evidence 不合格；主编保留祠堂耳房、林崇反问与林彻等待失算，补齐 evidence 并清掉机械回环，正文 1826 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch865-林叙等.md`：Claude 首稿虽过基础 lint，但缺 review 证据并残留禁词与模板回声；主编重写为林叙只记送达与“未收”，将凉粥、糖纸、旧铜扣分栏，正文 1541 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch866-林崇信.md`：Claude 首稿命中方向、自指与物象回声，且 hook 与正文不一致；主编重写为林崇把未封信留在案角，分开家册、老秤、交接纸与收件空栏，正文 1556 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch867-灶边雪.md`：Claude 首稿命中 30 处方向、17 处墙式和 75 处物象回声；主编重写为灶房三人分开东堂交接纸、兰帕旁证与热粥，林夙只记“原信未收”，正文 1712 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch868-林彻站.md`：Claude 首稿命中 19 处方向、3 处墙式和 66 处物象回声；主编重写为林彻在父亲与门外林夙之间落见件印，不替任何人完成收件，正文 1631 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch869-苏挽在.md`：Claude 首稿有可用的“还”字场景，但独立门抓到墙式回声并过度堆叠笔、帕、糖纸；主编重写为苏挽分开抄件、旁证与旧笔，不把人情写成归还，正文 1608 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch870-林叙看.md`：Claude 首稿缺有效 review/score 且沿用位置与自指模板；主编重写林叙核对东堂、灶房、余伯院三张纸，只记录看见的动作，保留林夙未收与赴山门的悬念，正文 1513 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch873-苏挽端糖.md`：Claude 首稿仍复写笔、帕、糖纸槽位且缺有效 review；主编重写为三块药糖的真实交接，余伯取用一块却不替苏挽把谢意变成签收，正文 1517 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch874-林彻看林夙.md`：Claude 首稿基础 lint 失败且沿用秤/门槛模板；主编重写为林夙经手老秤到西门、林彻记录药材见件，未封原信继续留在东堂，正文 1635 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch875-林叙等.md`：Claude 首稿虽过基础 lint 但缺 review，且把糖、帕、秤堆成声线模板；主编重写为林叙等林夙决定，把糖匣与未封原信分隔，记录未收而不代收，正文 1550 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch877-灶边雪.md`：Claude 首稿虽过基础门但仍有物象位置回声且 cast/关系不必要；主编重写为林夙、苏挽、阿湄核对焦纸、药包与黑牌，不把未验纸角投入炉火，正文 1534 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch878-林彻站.md`：Claude 首稿在 420 秒边界留下约十千字半稿，物象位置与自指回环达到 64/20；主编删除未收束稿并重写为林彻在东堂核对退婚书、未封原信与见件栏，把原信与灶灰分开，正文 1907 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch879-苏挽在.md`：Claude 首稿未能在预算边界内完成，原稿仍有方向、自我修复与物象回环；主编重写为苏挽留守药庐，清点药包、收回东堂旁纸，把等待落到药罐与值守记录，正文 1803 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch880-林叙看.md`：Claude 首稿虽有可读场景但缺少有效 review 证据，且把 ch881 的秤局提前重复；主编重写为林叙接住药庐旁纸与糖匣，林夙见匣不取，林崇移出老秤并命人请林彻，正文 1768 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch882-林彻站.md`：Claude 首稿残留 34 处方向公式、3 处墙式、58 处物象回环和 30 处自指；主编重写为林彻核对嫁妆契、母亲旧玉与林夙见件栏，把玉留在空盏而不代替任何人落名，正文 1793 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch883-苏挽端糖.md`：Claude 首稿虽过基础门但缺少有效 review/score 证据，且复写笔、糖纸和自指模板；主编重写为苏挽接住林夙带回的“玉落空盏”旁纸，分开药糖、旁证与余伯取用记录，正文 1521 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch884-林彻看林夙.md`：Claude 首稿残留 27 处 wall 公式并以糖物象重复解释；主编整章重写为林彻核对林夙转交的药糖、玉的旁纸与半印收存条，把碎糖吐进砚台但不让糖替纸落名，正文 1520 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch885-林叙等.md`：Claude 首稿在 420 秒边界留下 26 处方向公式与 48 处物象回环；主编重写为林叙接住林彻的糖入砚记录与库房旧信，等待林崇承担拆信决定，不把廊外脚步补成林夙回话，正文 1560 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch886-林崇信.md`：Claude 首稿基础门通过但严格门缺有效 review 证据，且重复退信/秤的旧解释；主编重写为林崇拆开林夙十三岁旧信，拒绝王姨娘半印退回，让林彻只作见证，信留砣底，正文 1536 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch887-灶边雪.md`：Claude 首稿有可用灶边现场但仍残留 2 处墙式、10 处自指和多处“那一X”回声；主编压缩重写为林夙在余伯院分开信留砣底旁证、黑蜡、糖粉与雪印，带空白栏去东堂，正文 1673 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
...
- `ch894-林彻看林夙.md`：Claude 基础 lint 通过但缺 review/score 证据；主编重写为林彻核对无封信匣、蓝线夹黑砂与门房抄件，拒绝把林夙去处写成亲见，留下“林彻亲启”，正文 1672 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch895-林叙等.md`：Claude 改写后仍有 2 处方向公式和 34 处物象槽位回环；主编重写为林叙把林夙入东堂、林彻未入营分成两份抄件，保留来源与收件空栏，再将信匣送往东堂，正文 1604 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch899-苏挽在.md`：Claude 基础 lint 通过但缺 review 证据，且正文仍有朝向、位置与自指模板；主编重写为苏挽登记旧笔、帕、铜秤和两行报信，保留第三行空白并把盖碗收回桌内，正文 1579 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch901-林崇看.md`：Claude worker 超时失败，回执记录 2 处墙式、21 处自指和 side-effect 文件；主编重写为林崇核对两行抄件、王姨娘灶灰残纸与林彻缺席见证，将信匣送往灶房但不补第三行，正文 1624 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch907-灶边雪.md`：Claude 首稿命中 1 处方向公式、8 处墙式与 31 处物象槽位，且 ships 误写为林夙×林夙；主编重写为林夙带着“林夙收”来信停在余伯院外，苏挽保存送达与血布证据，信始终未拆，正文 1536 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch926-林崇信.md`：Claude 首稿命中 25 处方向、13 处自我修复、7 处墙式、109 处物象槽位与 34 处自指；主编重写为林崇撕下冬账“叶清梧·还”一行，旧秤交接失手落砣，焦纸和半印分开存档，正文 1577 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch932-灶边.md`：Claude 首稿仍命中 3 处 §七.1 公式且缺 review 证据；主编重写为余伯在灶边交糖盘与封存木盒，苏挽分开旧账和糖纸，林夙从糖纸取出带林崇半枚私印的赭色封线，木盒保持未开，正文 1581 字，最终 lint、公式扫描、strict editorial 与 safety PASS，Claude 回执保留 BLOCKED。
- `ch881-灶边雪.md`（alternate）：旧分支原稿为纯公式病体，且与已绿的 canonical `ch881-林崇看.md` 同号；主编保留分支、补齐 branch 元数据，重写为林夙接收雪湿信并分开送达条、灶灰和蓝纤维，正文 1600 字，最终 lint、公式扫描、strict editorial 与 safety PASS。
