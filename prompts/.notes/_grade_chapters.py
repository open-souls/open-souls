# -*- coding: utf-8 -*-
"""Chapter quality grader. Scores 7-dim prose + 14 rubric + 12 publishable."""
...
OUT = ROOT / "prompts" / ".notes" / "2026-09-04-quality-grades.json"
...
def grade_7d(body):
    ...
    d3 = 2 - (1 if explicit_thought else 0) - (1 if reveal_phrases else 0)
    d3 = max(0, d3)
    ...
    # D7 克制
    summary_words = re.findall(r"(那一刻她明白|这一刻他明白|原来这就是|她终于理解|他想这就是)", body)
...
def grade_rubric(body, hook_text, fm):
    han = re.findall(r"[一-鿿]", body)
    paragraphs = [p for p in re.split(r"\n\s*\n", body) if p.strip()]
    ...
    reversal = re.findall(r"(不[一-鿿]{1,3}但|不[一-鿿]{1,3}却|可是|然而|却|但)", body)
    r3 = 0
    if len(reversal) >= 4:
        r3 += 1
    if len(reversal) >= 10:
    ...
    push_pull = re.findall(r"(替她|替他|替自己|不替|看见|不替看见)", body)
    r4 = 0
    if len(push_pull) >= 4:
        r4 += 1
    if len(push_pull) >= 10:
...
def assess(path):
    ...
    d7 = grade_7d(body)
    rubric = grade_rubric(body, hook_text, fm)
...
def main():
    ...
    n = len(grades)
    n_short = sum(1 for g in grades if g["body_chars"] < 1500)
    print(f"graded {n} chapters; {n_short} short", file=sys.stderr)

    d7_dist = Counter(g["d7_sum"] for g in grades)
    rubric_dist = Counter(g["rubric_sum"] for g in grades)
    pub_dist = Counter(g["pub_sum"] for g in grades)
    total_dist = Counter(g["total_27"] for g in grades)
    ...
    summary = {
        "n_chapters": n,
        "n_short_lt1500": n_short,
        "d7_dist": dict(d7_dist),
        "rubric_dist": dict(rubric_dist),
        "pub_dist": dict(pub_dist),
    ...
    print("\n=== Top 20 by total_27 ===", file=sys.stderr)
    top = sorted(grades, key=lambda g: -g["total_27"])[:20]
    for g in top:
        print("  %-30s d7=%2d/14  rubric=%2d/14  pub=%2d/12  total=%2d/27" % (
            g["file"][:30], g["d7_sum"], g["rubric_sum"], g["pub_sum"], g["total_27"]), file=sys.stderr)

    print("\n=== Bottom 20 ===", file=sys.stderr)
    bot = sorted(grades, key=lambda g: g["total_27"])[:20]
    for g in bot:
        print("  %-30s d7=%2d/14  rubric=%2d/14  pub=%2d/12  total=%2d/27 chars=%d" % (
            g["file"][:30], g["d7_sum"], g["rubric_sum"], g["pub_sum"], g["total_27"], g["body_chars"]), file=sys.stderr)
