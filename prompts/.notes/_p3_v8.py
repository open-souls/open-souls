# -*- coding: utf-8 -*-
"""P3 v8: accept shorter anchors (5+ chars) for fragmented cases."""
...
def pick_anchor(body_for_match, body_raw):
    ...
    if not paragraphs:
        return ""
    # Strategy A: walk backwards, try sentences at various lengths (down to 8)
            ...
            for length in [30, 25, 20, 15, 12, 10, 8]:
                if len(sent_clean) >= length:
                    ...
                    if candidate in body_for_match:
                        return candidate
    # Strategy B: take last paragraph directly if >= 5 chars
    for para in reversed(paragraphs[-5:]):
        para_clean = re.sub(r"\*\*", "", para).strip()
        if len(para_clean) >= 5 and para_clean in body_for_match:
            return para_clean[:30]
    # Strategy C: try shorter (5-7 char) substrings of single sentences
    for para in reversed(paragraphs[-10:]):
        para_clean = re.sub(r"\*\*", "", para).strip()
        sents = re.split(r"(?<=[。！？!?])", para_clean)
        sents = [s.strip() for s in sents if s.strip()]
...
def main():
    ...
    print("P3 v8:", len(fixed), "fixed,", len(failed), "failed")
