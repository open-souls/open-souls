# -*- coding: utf-8 -*-
"""P3 v5: handle fragmented single-sentence paragraphs."""
...
def pick_anchor(body_for_match, body_raw):
    """Pick 8-30 char anchor that IS verbatim in body_for_match.

    1. Try taking 30/25/20 chars from last sentence (split by 。！？!?).
    2. If none of those exist in body, try joining the last 2-4 short sentences.
    """
            ...
            for length in [30, 25, 20, 15, 12, 10, 8]:
                if len(sent_clean) >= length:
                    ...
                    if candidate in body_for_match:
                        return candidate
    # Strategy B: join last 2-4 short sentences from last paragraph(s)
    last_lines = []
    for para in reversed(paragraphs[-6:]):
        para_clean = re.sub(r"\*\*", "", para).strip()
        sents = re.split(r"(?<=[。！？!?])", para_clean)
        sents = [s.strip() for s in sents if s.strip()]
        if sents:
            last_lines.insert(0, sents[-1].rstrip("。！？!?\n\"\'「」"))
        if len(last_lines) >= 4:
            break
    merged = "".join(last_lines)
    for length in [30, 25, 20, 15, 12, 10, 8]:
...
def main():
    ...
    print("P3 v5:", len(fixed), "fixed,", len(failed), "failed")
