# -*- coding: utf-8 -*-
"""P3 v4: pick_anchor returns only verbatim body substrings."""
...
def pick_anchor(body_for_match, body_raw):
    """Pick 8-30 char anchor that IS a verbatim substring of body_for_match.

    Strategy:
    - Take last 8 paragraphs (skipping title/stars)
    - From each, take sentences (split by 。！？!?)
    - For each sentence: try first 30 chars, then 25, 20, 15 chars
    - Return the first that exists verbatim in body_for_match
    """
    text = body_raw
    ...
    for para in reversed(paragraphs[-8:]):
        ...
        if not sents:
            continue
        # try the last sentence first, then earlier ones
        for sent in reversed(sents):
            sent = sent.rstrip("。！？!?\n\"\'「」")
            for length in [30, 25, 20, 15, 12, 10]:
                if len(sent) >= length:
                    candidate = sent[:length]
                    if candidate in body_for_match:
                        return candidate
    # fallback: try concatenating last 2 sentences
    if len(paragraphs) >= 2:
        last2 = paragraphs[-2:]
        for para in last2:
            para_clean = re.sub(r"\*\*", "", para).strip()
            sents = re.split(r"(?<=[。！？!?])", para_clean)
            sents = [s.strip() for s in sents if s.strip()]
            for sent in reversed(sents[-2:]):
                sent = sent.rstrip("。！？!?\n\"\'「」")
                for length in [25, 20, 15, 10]:
                    if len(sent) >= length:
                        candidate = sent[:length]
                        if candidate in body_for_match:
                            return candidate
...
def main():
    ...
    print("P3 v4:", len(fixed), "fixed,", len(failed), "failed")
