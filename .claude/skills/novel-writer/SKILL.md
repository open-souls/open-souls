...
- Narrow-handoff failure rule: if a single-file, no-Bash handoff still runs past about 60 seconds, reports `aborted_tools`, shows unexpectedly large input/context, and leaves the target unchanged, classify the failure as the Claude harness/route rather than a prompt-quality issue. Do not launch two concurrent retries after the circuit breaker is active; allow at most one minimal worker in a later iteration, then take over locally and record the measured evidence.
